...
public protocol FlowGraphType {
    ...
    typealias WaitingFlowHandler<Flow> = (Event, Flow) -> WaitingStateOut<
        WaitingState, RunningState, Event
    >
...
private typealias WaitingAnyHandler<T: FlowGraphType> = (T.Event, Any) -> WaitingStateOut<
    T.WaitingState, T.RunningState, T.Event
>

private enum Waiting<T: FlowGraphType> {
...
public class FlowGraphBuilder<T: FlowGraphType> {
    ...
    public func add<SubFlow: Initializable>(
        waiting state: T.WaitingState,
